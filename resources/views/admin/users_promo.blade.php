...
@section('scripts')
    ...
    <script>
        ...
        $(function() {
            var new_table = $('#users-table').DataTable({
                ...
                ajax: {
                    ...
                    url: "{{ route('promousers.datatable') }}",
                ...
                columns: [
                    ...
                    { data: 'promo_amount', name: 'promo_amount' },
        ...
        // INSERT CREDIT WISH START

       function credit_wish(id,amount,mt4) {

           swal({
               title: "Are you sure?",
               text: "Do you want to add the amount to user?",
               icon: "warning",
               buttons: true,
               dangerMode: true,
           }).then((result) => {
               if (result) {
                   axios.post('{{URL::to('admin/insert_credit_wish')}}', {
                       id:id,
                       amount:amount,
                       mt4:mt4,
                   }).then(function(response) {
                       // console.log(response);
                       toastr.success("Collateral inserted!", {
                           positionClass: 'toast-bottom-right',
                           containerId: 'toast-bottom-right'
                       });
                       // window.location.reload();
